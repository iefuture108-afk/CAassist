# app.py - CA Deployer with Voice Input & Dark Mode
...
# Custom CSS (changes based on dark_mode)
def get_css(dark):
    if dark:
        return """
        <style>
...
function startVoiceInput() {
    ...
    recognition.onresult = (event) => {
        ...
        // Set the value in the Streamlit text input (using id from querySelector)
        const inputField = parent.document.querySelector('input[data-testid="stTextInput"]');
        if (inputField) {
            inputField.value = spokenText;
            // Trigger an input event so Streamlit detects the change
            inputField.dispatchEvent(new Event('input', { bubbles: true }));
        }
        // Also click the send button (optional)
...
<button onclick="startVoiceInput()" style="background:#007bff; color:white; border:none; border-radius:20px; padding:0.3rem 0.8rem; cursor:pointer;">
🎤 Speak your question
</button>
"""
...
def show_penalty_calculator():
    ...
    col1.metric("Late Fee (₹50/day)", f"₹{late_fee:,.0f}")
    col2.metric("Interest (18% p.a.)", f"₹{interest:,.0f}")
    col3.metric("Total Penalty", f"₹{total:,.0f}", delta="Jaldi bharein")
...
# ---------- Sidebar with Mascot + Dark Mode Toggle ----------
with st.sidebar:
...
# ---------- Voice input button placed below content (easy access) ----------
st.markdown("---")
st.markdown("#### 🎤 या सीधे बोलकर सवाल पूछें:")
components.html(voice_html, height=60)  # duplicate for convenience
